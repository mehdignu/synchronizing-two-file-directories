...
def sync(source: Path, destination: Path):
    ...
    for folder, _, files in os.walk(destination):
        for file_name in files:
            ...
            elif destination_hash in source_hashes and source_hashes[destination_hash] != file_name:
                shutil.move(destination_path, Path(folder)/source_hashes[destination_hash])
